...
func (c landCapability) Definition() Definition {
	return Definition{
		...
		Description: "Merge the pull request already bound to this issue, repository, base, and branch using the configured merge method, once required checks pass, reviews have no effective changes-requested state, and no review thread is unresolved. A pull request that is merely behind the base branch is landed as-is; the base moving while this call runs is what is gated. Returns a non-terminal waiting result while required checks or GitHub's mergeability computation are pending; with github.update_stale_branch enabled, one clean update-branch merge for a base that moved mid-landing also waits for checks on its new head. A waiting result ends this run and Symphony itself redispatches landing later; a merged result ends this run for good. Never call this tool again after either outcome. Other hard gates (failing checks, requested changes, unresolved threads, a base that moved mid-landing, conflicts, or a closed/mismatched PR) refuse landing. No repository, issue, branch, PR, method, state, or credential input.",
		InputSchema: map[string]any{"type": "object", "additionalProperties": false},
	}
}
